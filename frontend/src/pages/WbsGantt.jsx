  ...
  const handleCreateSubmit = (e) => {
    ...
    wbsService.createTask({
      ...
      .then(() => {
        ...
        setFormData({ item_type: 'Phase', parent_phase_id: '', parent_task_id: '', wbs_code: '', title: '', contractor_name: '', start_date: new Date().toISOString().split('T')[0], end_date: new Date(Date.now() + 30 * 86400000).toISOString().split('T')[0], planned_qty: '', planned_budget: '' });
        loadWbs(selectedProjectId);
      })
                          ...
                          return (
                            <React.Fragment key={`task-${task.id}`}>
                              <tr style={{ background: 'rgba(245,158,11,0.03)', borderLeft: '4px solid var(--accent-amber)' }}>
                                ...
                                <td><span className="tag-badge tag-warning">Task</span></td>
                              ...
                              {!isTaskCollapsed && task.subtasks.map((subtask) => {
                                ...
                                return (
                                  <tr key={`subtask-${subtask.id}`} style={{ background: 'rgba(16,185,129,0.02)', borderLeft: '4px solid var(--accent-emerald)' }}>
                                    ...
                                    <td><span className="tag-badge tag-success">Subtask</span></td>
            ...
            <form onSubmit={handleCreateSubmit}>
              <div className="form-group">
                ...
                <select className="form-select" value={formData.item_type} onChange={e => setFormData({ ...formData, item_type: e.target.value })}>
                  <option value="Phase">Phase (L1 Top-Level)</option>
                  <option value="Task">Task (L2 Work Package)</option>
                  <option value="Subtask">Subtask (L3 Task)</option>
                </select>
              </div>
                ...
                <div className="form-group">
                  ...
                  <select required className="form-select" value={formData.parent_phase_id} onChange={e => setFormData({ ...formData, parent_phase_id: e.target.value })}>
                    ...
                    {availablePhases.map(p => (
                      <option key={p.id} value={p.id}>{p.wbs_code ? `${p.wbs_code} ` : ''}{p.title}</option>
                    ))}
                  </select>
                </div>
              )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '1rem' }}>
                <div className="form-group">
                  ...
                  <input type="text" className="form-control" placeholder="e.g. 1.1.1" value={formData.wbs_code} onChange={e => setFormData({ ...formData, wbs_code: e.target.value })} />
                </div>
                <div className="form-group">
                  <label>Title / Task Name</label>
                  <input required type="text" className="form-control" placeholder="e.g. Master Schedule Development (CPM)" value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} />
                </div>
              </div>
              ...
              <div className="form-group">
                <label>Planned Quantity (Optional)</label>
                <input type="number" step="any" className="form-control" placeholder="e.g. 5000" value={formData.planned_qty} onChange={e => setFormData({ ...formData, planned_qty: e.target.value })} />
              </div>
          ...
          <div className="glass-card" style={{ width: '560px', background: '#0f172a', border: '1px solid var(--border-color-hover)' }}>
            ...
            <form onSubmit={handleEditSubmit}>
              ...
              <div className="form-group">
                ...
                <input type="number" step="any" className="form-control" placeholder="e.g. 5000" value={editFormData.planned_qty} onChange={e => setEditFormData({ ...editFormData, planned_qty: e.target.value })} />
              </div>
